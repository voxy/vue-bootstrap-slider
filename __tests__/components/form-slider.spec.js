...
describe('form-slider', async() => {
    ...
    it.only('should add custom classes', async () => {
        const {app: {$refs, $el}} = window;

        const vm = $refs.custom;

        const trackSlider = vm.$el.querySelector('.slider-track');
        expect(trackSlider).toHaveClass('.secondary-background-color');
    })
});
